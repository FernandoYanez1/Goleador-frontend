...
export default function Placar() {
    ...
    const [teams, setTeams] = useState<any[]>([]); // Apenas para aposta de campeão
    ...
    // Botão Voltar: Só exibe se houver MAIS de uma rodada aberta, para permitir voltar ao Lobby
    const handleVoltar = () => {
        if (rodadas.length > 1) {
            setRodadaAtiva(null);
        } else {
                                        ...
                                        <Card style={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'between', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', border: eCampeao ? '2px solid #fbbf24' : 'none', backgroundColor: '#1e293b', color: 'white' }}>
                                            ...
                                            <CardActions style={{ padding: '20px', paddingTop: 0 }}>
                                                <Button 
                                                    ...
                                                    style={{ backgroundColor: eCampeao ? '#fbbf24' : '#3b82f6', color: eCampeao ? '#92400e' : 'white', fontWeight: 'bold', padding: '10px' }} 
                                                >
                                                    {eCampeao && tempoRestante.expirado ? "Inscrições Fechadas" : "Escolher Bilhete"}
                                                </Button>
                                            </CardActions>
                                        </Card>
                                    </Grid>
                                ...
                                <Grid container alignItems="center" justifyContent="center" spacing={1}>
                                    ...
                                    <Grid item xs={4} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                                        <Box display="flex" alignItems="center" gap={1}>
                                            <Typography fontWeight="bold" fontSize="18px">{jogo.sigla_casa}</Typography>
                                            <img src={jogo.logo_casa || "/media/escudos-times/default.png"} alt="casa" style={{ width: 40, height: 40, objectFit: 'contain' }} />
                                        </Box>
                                        <Typography variant="caption" color="#64748b" style={{ marginTop: '4px' }}>{jogo.time_casa}</Typography>
                                    </Grid>
                                    
                                    {/* Inputs de Placar (Limpos) */}
                                    <Grid item xs={4} textAlign="center" display="flex" justifyContent="center" alignItems="center" gap={2}>
                                        <input 
                                            type="number"
                                            value={palpitesPlacares[jogo.id]?.casa ?? ''}
                                            onChange={(e) => handlePlacarChange(jogo.id, 'casa', e.target.value)}
                                            style={{ width: '50px', height: '45px', textAlign: 'center', fontSize: '20px', fontWeight: 'bold', borderRadius: '8px', border: '2px solid #cbd5e1', backgroundColor: '#f8fafc', color: '#1e293b', outline: 'none' }}
                                        />
                                    ...
                                    <Grid item xs={4} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                                        <Box display="flex" alignItems="center" gap={1}>
                                            ...
                                            <Typography fontWeight="bold" fontSize="18px">{jogo.sigla_visitante}</Typography>
                                        </Box>
                                        <Typography variant="caption" color="#64748b" style={{ marginTop: '4px' }}>{jogo.time_visitante}</Typography>
                                    </Grid>
